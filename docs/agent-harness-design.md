...
| `SessionStore` | Save, load, load latest, and fork session snapshots. | Memento, Prototype |
...
| Provider selector/registry | `--model provider/model` | `openai`, `openrouter`, future providers |
...
Agent selection is also a registry lookup. Agent presets share the runner while changing instructions, enabled tools, and limits:
...
| Adapter | Implemented | OpenAI-compatible provider adapter; custom-SDK adapters are extension points |
...
4. Add JSON-backed `SessionStore` with create, save, load, load-latest, and fork.
